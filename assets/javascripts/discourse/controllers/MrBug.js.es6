export default Ember.Controller.extend({
	...
	actions: {
		...
		imgoingin() {
			...
			Ember.$.ajax({
				url: "/MrBug/zaips/"+btoa(this.get('prezaips.position')+"~"+this.get('currentUser.username')+"~"+this.get('prezaips._id')+"~"+encodeURIComponent(this.get('prezaips.gameNAME')))+".json",
				type: "GET"
			}).then(result => {
