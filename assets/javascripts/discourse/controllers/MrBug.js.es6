export default Ember.Controller.extend({
	...
	actions: {
		...
		troikopoisk() {
			...
			Ember.$.ajax({
				url: "/MrBug/troikopoisk/"+btoa(unescape(encodeURIComponenet(this.get('troikopoisk2'))))+".json",
				type: "GET"
			}).then(result => {
		...
		imgoingin() {
			...
			Ember.$.ajax({
				url: "/MrBug/zaips/"+btoa(unescape(encodeURIComponenet(this.get('prezaips.position')+"~"+this.get('currentUser.username')+"~"+this.get('prezaips._id')+"~"+this.get('prezaips.gameNAME'))))+".json",
				type: "GET"
			}).then(result => {
