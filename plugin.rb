...
after_initialize do
	...
	class ::MrbugController < ::ApplicationController
		...
		db = Mongo::Client.new([ SiteSetting.site_ip+':17113' ], user: 'troiko_user', password: '47TTGLRLR3' )
		...
		def show
			...
			if autozCache.blank? || (current_user && current_user[:username] == 'H1tomaru') #megaaddhack
				...
				gameDB.each do |game|
					...
					if users
						...
						for i in 0..troikaNO
							...
							if current_user && current_user[:username] == 'H1tomaru' #megaaddhack
				...
				@@cachedb[:autozCache].insert_one( { gamelist: gameDB, TIME: Time.now } ) unless current_user && current_user[:username] == 'H1tomaru' #megaaddhack
		...
		def zapass
			...
			user_BGZ = @@userdb[:PS4db].find( 
				{ _id: Base64.decode64(params[:myylo]), "$or": [ { P2: user_d }, { P4: user_d } ] },
			...
			if current_user && params[:myylo] && user_d == params[:username].downcase && !user_BGZ.blank?
				...
				if inputfb && inputfb[:FEEDBACKS] && inputfb[:troikaBAN] == 0
		...
		def rentagama
			...
			respond_to do |format|
				...
				format.json { render json: rentaCache } else { render "default/custom" }
			end
