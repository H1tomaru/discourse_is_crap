	...
	class ::MrbugController < ::ApplicationController
			...
			if gamelist.empty?
				...
				gameDB.each do |game|
					...
					if game[:PRICE] > 0
						...
						p4UP = [0,100,50] if game[:PRICE] < 2601
						p4UP = [0,100,0] if game[:PRICE] < 1001
						p4UP = [50,50,0] if game[:PRICE] < 501
		...
		def rentagama
			...
			rentagamez.each do |games|
				...
				( hideoz = false; count[5] = count[5] + 1 ) if current_user && rentahideo[0] && rentahideo[0][games[:_id]] && rentahideo[0][games[:_id]] == false
			...
			if current_user
				...
				if hideos.length == 2 && current_user[:username].downcase == hideos[0].downcase
					...
					if rentahideo[0]
						...
						@@rentadb[:rentahideo].find_one_and_update( { _id: current_user[:username].downcase }, { "$set": { hideos[1] => value } }, { upsert: true } )
						render json: { HiMom: "!!!!" }
					else
						@@rentadb[:rentahideo].insert_one( { _id: current_user[:username].downcase, DATE: Time.now.strftime("%Y.%m.%d"), hideos[1] => false } )
						render json: { HiMom: "!!!!" }
					end
