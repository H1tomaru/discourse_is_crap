			...
			if current_user
				...
				if hideos.length == 2 && current_user[:username].downcase == hideos[0].downcase
					...
					if rentahideo[0]
						...
						@@rentadb[:rentahideo].find_one_and_update( { _id: current_user[:username].downcase }, { "$set": { hideos[1].gsub(".", "\u002e") => value } }, { upsert: true } )
						render json: { HiMom: "!!!!" }
					else
						@@rentadb[:rentahideo].insert_one( { _id: current_user[:username].downcase, DATE: Time.now.strftime("%Y.%m.%d"), hideos[1].gsub(".", "\u002e") => false } )
						render json: { HiMom: "!!!!" }
					end
