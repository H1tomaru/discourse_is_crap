...
after_initialize do
	...
	class ::MrbugController < ::ApplicationController
		...
		def troikopoisk
			#decode shit
			troikopoisk = URI.escape(Base64.decode64(params[:input])).strip.downcase
		...
		def zaips
			#decode shit
			code = URI.escape(Base64.decode64(params[:bagatrolit])).split("~") #0 - position, 1 - userNAME, 2 - gameCODE, 3 - gameNAME
		...
		def zafeedback
			#decode shit
			fedbacks = URI.escape(Base64.decode64(params[:fedbakibaki])).split("~") #0 - score, 1 - otziv
